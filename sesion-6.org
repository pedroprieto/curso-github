...
#+BEGIN_SRC yaml
 ...
 gem install travis 
 #+END_SRC
 - Si se está utilizando el servicio travis-ci.com, iniciar sesión en la aplicación Travis mediante el usuario y el *token* de GitHub (debes haber creado el [[./sesion-3.org][token de acceso]] previamente). Para ello ejecutar:
 #+BEGIN_SRC bash
