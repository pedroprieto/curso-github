...
2. Habilitar el servicio de GitHub Pages en el repositorio. Para ello, en los ajustes del repositorio (/Settings/), en la sección /Pages/, se debe marcar la opción /Deploy from a branch/. (También está la opción de publicar a través de GitHub Actions, servicio que veremos en el siguiente tema).
3. A continuación, se debe *elegir una rama* del repositorio. El contenido de dicha rama es el que se publicará. Se puede elegir cualquiera de las ramas disponibles en el repositorio. En este punto, una posibilidad es dedicar una rama a alojar el sitio web y el resto al código fuente del proyecto. No obstante, si se va a utilizar el repositorio exclusivamente como servidor web, se puede utilizar únicamente una rama.
   [[./imagenes/pages.png]] 
5. Opcionalmente, se puede seleccionar la opción de *dedicar una carpeta* de la rama para la publicación. Así:
   - Si se elige la opción /root/, se publicará todo el contenido del repositorio.
   - Si se elige la opción /docs/, se publicará únicamente el contenido de dicha carpeta.
6. Los archivos que se creen estarán disponibles en la dirección: https://MINOMBREDEUSUARIO.github.io/nombrerepositorio.
